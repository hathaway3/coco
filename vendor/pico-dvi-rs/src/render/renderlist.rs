...
impl RenderlistBuilder {
    ...
    pub fn end_stripe(&mut self) {
        self.v.push(render_stop as *const () as u32);
        let len = self.v.len();
    ...
    pub fn tile64(&mut self, tile: &[u32], start: u32, mut end: u32) {
        ...
        if start == 0 && end == 16 {
            ...
            if offset == 0 {
                self.v
                    .extend([render_blit_64_aligned as *const () as u32, tile.as_ptr() as u32, stride]);
            } else {
                let off4 = offset * 4;
                let shift = off4 + ((32 - off4) << 8);
                self.v.extend([
                    render_blit_64_straddle as *const () as u32,
                    tile.as_ptr() as u32,
                    stride,
                    shift,
                ]);
    ...
    pub fn text(&mut self, text: &str) -> u32 {
        ...
        if !text.is_empty() {
            ...
            if self.v[op_ix] == render_blit_simple as *const () as u32 {
                self.v[op_ix] = render_blit_out as *const () as u32;
            } else if self.v[op_ix] == render_blit_straddle as *const () as u32 {
                self.v[op_ix] = render_blit_straddle_out as *const () as u32;
            }
        }
        x
    }
    ...
    pub fn blit_1bpp(&mut self, array: &[u32], words: usize, stride: u32) {
        ...
        self.v.extend(
            core::iter::repeat(render_blit_out as *const () as u32)
                .take(words)
                .enumerate()
