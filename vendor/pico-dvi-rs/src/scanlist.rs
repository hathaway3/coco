...
impl ScanlistBuilder {
    ...
    pub fn end_stripe(&mut self) {
        self.v.push(video_scan_stop as *const () as u32);
    }

    /// Generate a run of solid color.
    pub fn solid(&mut self, count: u32, color: u32) {
        self.v
            .extend_from_slice(&[video_scan_solid_16 as *const () as u32, count, color]);
        self.x += count;
    }
    ...
    pub fn pal_1bpp(&mut self, count: u32, palette: &Palette1bpp) {
        self.v.extend_from_slice(&[
            video_scan_1bpp_pal_16 as *const () as u32,
            count,
    ...
    pub unsafe fn pal_4bpp(&mut self, count: u32, palette: &Palette4bppFast) {
        self.v.extend_from_slice(&[
            video_scan_4bpp_pal_16 as *const () as u32,
            count,
            palette as *const _ as u32,
        ]);
        self.x += count;
    ...
    pub fn copy_pixels(&mut self, count: u32) {
        self.v
            .extend_from_slice(&[video_scan_copy_16 as *const () as u32, count, 0]);
        self.x += count;
    }
}
